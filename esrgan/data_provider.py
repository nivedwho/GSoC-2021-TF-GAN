...
def download_hr(mode, target_dir):
  ...
  tf.keras.utils.get_file(filename, source_url,
                          cache_subdir=target_dir, extract=True)
...
def get_div2k_data(HParams,
  ...
  ds = tf.data.Dataset.zip((lr_data(mode, HParams.data_dir, download_lr_data),
                            hr_data(mode, HParams.data_dir, download_hr_data)))
  ds = ds.map(lambda lr, hr: scale(lr, hr, HParams.hr_dimension, HParams.scale),
              num_parallel_calls=AUTOTUNE)
